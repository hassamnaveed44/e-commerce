...
export default function OrderDetailView({ orderId }: { orderId?: string }) {
  ...
  if (isLoading) {
    return (
      <div className="py-24 flex flex-col items-center justify-center gap-2 text-slate-500">
        <RefreshCw size={22} className="animate-spin text-slate-800" />
  ...
  return (
    <div className="space-y-6 pb-20 font-satoshi text-slate-900 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={handlePrint}
            className="border border-slate-200 bg-white hover:bg-slate-50 text-slate-800 px-3.5 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="bg-black text-white hover:bg-black/85 px-4 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-xs"
          >
            <Edit2 size={13} />
            <span>Edit</span>
          </button>
      ...
      {/* 2️⃣ Top Grid: Order Info & Order Summary (Screenshot 4 Match) */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
        {/* Left Card: Order ORD-XXXXX & Customer Info (Screenshot 4 Match) */}
        <div className="md:col-span-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-5">
          {/* Order Header */}
          <div>
          ...
          {/* Payment Method Sub-Box (Screenshot 4 Match) */}
          <div className="rounded-xl bg-slate-50 border border-slate-200/80 p-3.5 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => setIsEditModalOpen(true)}
              className="w-7 h-7 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 flex items-center justify-center text-slate-500 transition cursor-pointer shadow-2xs"
              title="Edit Payment"
            >
              <Edit2 size={12} />
            </button>
        ...
        {/* Right Card: Order Summary (Screenshot 4 Match) */}
        <div className="md:col-span-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between space-y-6">
          <div>
      ...
      {/* 3️⃣ Delivery Status (Step Tracker - Screenshot 4 Match) */}
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-6">
        ...
        <div className="relative pt-2 pb-2">
          ...
          <div className="absolute top-7 left-8 right-8 h-1 bg-slate-200 -translate-y-1/2 z-0">
            <div
        ...
        <div className="pt-2">
          <span className="inline-flex items-center gap-1.5 text-xs text-slate-600 font-medium">
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold border border-blue-200 bg-blue-50 text-blue-700">
              {order.orderStatus}
            </span>
      ...
      {/* 4️⃣ Order Items Table (Screenshot 4 Match) */}
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="py-3.5 px-2">
                      ...
                      >
                        ...
                        <div>
                          <span className="font-semibold text-slate-900 group-hover/item:underline block text-xs">
                            {item.productName}
                          </span>
                    ...
                    {/* Price */}
                    <td className="py-3.5 px-4 text-right font-medium text-slate-800 font-mono">
          ...
          <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-2xl p-5 text-slate-900 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <button
                ...
                className="w-7 h-7 rounded-full bg-slate-100 text-slate-600 hover:text-slate-900 flex items-center justify-center transition"
              >
                <X size={14} />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              <select
                ...
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-xs bg-white text-slate-900 outline-none font-semibold cursor-pointer"
              >
            ...
            <div className="pt-2 flex items-center justify-end gap-2">
              ...
              <Button
                ...
                disabled={isUpdating}
                className="bg-black text-white rounded-lg text-xs font-semibold"
              >
