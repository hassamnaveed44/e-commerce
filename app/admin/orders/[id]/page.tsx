  ...
  const fetchOrder = async () => {
    try {
      ...
      if (data.success && data.order) {
        ...
        // Fallback: try fetching all orders and pick first or matched
        const allRes = await fetch(`/api/admin/orders`);
        const allData = await allRes.json();
        if (allData.success && allData.orders?.length > 0) {
          const found = allData.orders.find(
            (o: any) => o.id === resolvedParams.id || o.orderNumber === resolvedParams.id
          );
          setOrder(found || allData.orders[0]);
          setNewStatus(found?.orderStatus || allData.orders[0].orderStatus);
        }
      }
    } catch (err) {
