...
import { ChevronRight, ChevronUp, SlidersHorizontal, Check, X } from "lucide-react";
...
export default function FiltersSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed lg:static inset-x-0 bottom-0 lg:translate-y-0 h-[85vh] lg:h-auto w-full lg:w-[295px] bg-white z-50 lg:z-0 p-6 rounded-t-[30px] lg:rounded-[20px] border border-black/10 overflow-y-auto transition-transform duration-300 ease-out shrink-0 ${
          isOpen ? "translate-y-0" : "translate-y-full lg:translate-y-0 hidden lg:block"
        }`}
      >
        {/* Mobile Drag Handle */}
        <div className="w-12 h-1.5 bg-black/20 rounded-full mx-auto mb-4 lg:hidden" />
        ...
        <div className="flex items-center justify-between pb-5 border-b border-black/10 mb-6">
          ...
          <div className="flex items-center gap-2">
            <button 
              type="button"
              className="w-8 h-8 rounded-full bg-[#F0F0F0] flex items-center justify-center text-black hover:bg-black/10 lg:hidden cursor-pointer" 
              onClick={onClose}
              aria-label="Close filters"
            >
              <X size={18} />
            </button>
            <SlidersHorizontal size={18} className="text-black/40 hidden lg:block" />
          </div>
        </div>

        {/* Categories List */}
        <div className="flex flex-col space-y-3.5 pb-6 border-b border-black/10 font-satoshi text-base">
          {categories.map((cat) => (
            <button
              key={cat}
              type="button"
        ...
        {/* Price Section - Dual Range Slider matching Figma */}
        <div className="py-6 border-b border-black/10 font-satoshi">
          <button
            type="button"
              ...
              {dressStyles.map((style) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between transition-colors text-left cursor-pointer ${
                    selectedDressStyle === style ? "text-black font-bold" : "text-black/60 hover:text-black"
                  }`}
                >
                  <span>{style}</span>
                  <ChevronRight size={16} className={selectedDressStyle === style ? "text-black" : "text-black/40"} />
                </button>
              ))}
            </div>
          )}
        ...
        <button 
          type="button"
          onClick={handleApply}
          className="w-full bg-black text-white rounded-full py-4 font-medium hover:bg-black/80 transition-colors text-center text-base mt-4 font-satoshi mb-6 lg:mb-0 cursor-pointer shadow-sm"
        >
          Apply Filter
        </button>
      </aside>
