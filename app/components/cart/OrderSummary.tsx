...
export default function OrderSummary() {
  ...
  const handleApplyPromo = (e: React.FormEvent) => {
    ...
    if (success) {
      setPromoMessage("Promo code applied successfully! (20% off)");
      setInputCode("");
    } else {
      setPromoMessage("Invalid promo code. Try SHOP20");
    }
  };
    ...
    <div className="border border-black/10 rounded-[20px] p-5 sm:p-6 bg-white font-satoshi shadow-xs">
      ...
      <form onSubmit={handleApplyPromo} className="flex items-center gap-3 mb-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={promoCode ? `Applied: ${promoCode}` : "Add promo code (e.g. SHOP20)"}
            value={inputCode}
      ...
      <Link
        ...
        className={`w-full rounded-full py-4 font-medium transition-colors flex items-center justify-center gap-2 text-base font-satoshi shadow-sm ${
          isCartEmpty
