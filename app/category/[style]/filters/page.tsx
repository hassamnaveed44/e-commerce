...
export default function MobileFiltersPage() {
  ...
  const styleParam = (params?.style || params?.category || "casual") as string;
  ...
  return (
    <main className="min-h-screen bg-white font-satoshi py-4 px-4 pb-12">
      <div className="max-w-md mx-auto bg-white rounded-[20px] border border-black/10 p-5 sm:p-6 shadow-xs">
        ...
        <div className="flex flex-col space-y-3.5 pb-6 border-b border-black/10 text-base">
          {categories.map((cat) => (
            <button
              ...
              className={`flex items-center justify-between transition-colors text-left ${
        ...
        {/* Apply Filter Button -> Navigates back to category */}
        <Link
          href={`/category/${styleParam}`}
          className="w-full block bg-black text-white rounded-full py-4 font-medium hover:bg-black/80 transition-colors text-center text-base mt-6 font-satoshi shadow-sm"
        >
          Apply Filter
        </Link>
      </div>
